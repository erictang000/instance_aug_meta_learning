...
#         #Map weights to grid
#         weights_grid=((weights - self.min.to(weights.device))/self.stride.to(weights.device)).to(torch.int32)

#         #On x-axis
#         ind = weights_grid[:,0]
#         x_translation_mat = torch.index_select(self.translation_tensor.to(ind.device), 0, ind).unsqueeze(-1) #[batch, size, size, 1]

#         # ind = weights_grid[:,2]
#         # x_zoom_mat = torch.index_select(self.zoom_tensor.to(ind.device), 0, ind).unsqueeze(1) #[batch, 1, size, size]
#         # x_mat = (x_translation_mat*x_zoom_mat).sum(2) #[batch, size, size]
#         x_mat = x_translation_mat.sum(2)
        
#         #On y-axis
#         ind = weights_grid[:,1]
#         y_translation_mat = torch.index_select(self.translation_tensor.to(ind.device), 0, ind).unsqueeze(-1) #[batch, size, size, 1]
        
#         # ind = weights_grid[:,3]
#         # y_zoom_mat = torch.index_select(self.zoom_tensor.to(ind.device), 0, ind).unsqueeze(1) #[batch, 1, size, size]
#         # y_mat = (y_translation_mat*y_zoom_mat).sum(2).transpose(1,2) #[batch, size, size]
#         y_mat = y_translation_mat.sum(2).transpose(1,2) 
#         print*
#         return x_mat, y_mat
...
#     img=plt.imread('./0000.jpeg')
#     img=torch.Tensor(np.transpose((img/255.0).astype(np.double), (2, 0, 1)))
...
#     plt.imsave("temp.png", output_new)
