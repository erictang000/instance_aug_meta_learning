    ...
    def forward(self, x, tta_only=False):
        ...
        if self.color_layer!=[]:
            ...
            if self.center_color==None:
                center_color=[self.conv.center(layer) for layer in self.color_layer][0][0][0]#?To be improved 
            else:              
...
class Augmentation(nn.Module):
    ...
    def __init__(self, conv, transform, cfg={}, device='cuda', tta_only=False):
        ...
        self.transform_crop=[t for t in transform if t in ['crop', 'erase']]
        ...
        if 'crop' in self.transform or 'erase' in self.transform:
    ...
    def forward_(self, x, n_copies=1, output_max=0, global_aug=False, random_crop_aug=False, random_color_aug=False, hsv_input=True, params=None):
        ...
        if params is not None:
            ...
            params_crop, param_color, center_color, param_rotation, param_crop_meanfield=self.get_param(x)
        ...
        if 'erase' in self.transform:
            ...
            weights_dim=Generators.weights_dim("crop")
            mat = getattr(Generators, "crop")(weights[:,i:i+weights_dim])
            if i==0:
                affine_matrices=mat
            else:
            ...
            a = affine_matrices[:,0,0]
            b = affine_matrices[:,1,1]
            c = affine_matrices[:,0,2]
            d = affine_matrices[:,1,2]
            x1 = -a + c
            x2 = a + c
            y1 = -b + d
            y2 = b + d
            w, h = x.shape[-2:]
            x1 = (w * (x1 + 1)/4).long().cpu().clip(0, w)
            x2 = (w * (x2 + 1)/4).long().cpu().clip(0, w)
            y1 = (h * (y1 + 1)/4).long().cpu().clip(0, h)
            y2 = (h * (y2 + 1)/4).long().cpu().clip(0, h)
            # import numpy as np
            # x_indexer = np.r_[tuple([np.s_[i:j] for (i,j) in zip(x1, x2)])]
            # y_indexer = np.r_[tuple([np.s_[i:j] for (i,j) in zip(y1, y2)])]

            x_out = x.clone()
            count = 0
            for j in range(x.shape[0]):
                if (x2[j] - x1[j]) *  (y2[j] - y1[j]) < 0.6 * w * h:
                    count += 1
                    x_out[j, :, x1[j]:x2[j], y1[j]:y2[j]] = 0
            # print(w * h * 0.4)
            # print((x2[j] -x1[j]), )
            # print((x2[j] - x1[j]) *  (y2[j] - y2[j]))
            # import matplotlib.pyplot as plt
            # print(x1[0], y1[0], x2[0], y2[0])
            # plt.imsave("grid_sample.png", np.transpose(x_out[0].detach().cpu().numpy(), (1, 2, 0)).clip(0, 1))
            # plt.imsave("recovered_box.png",  np.transpose(x[0, :, x1[0]:x2[0],y1[0]:y2[0]].cpu().numpy(), (1, 2, 0)).clip(0, 1))      
            
            return x_out, sample_logprob, entropy_every, KL_every, count/(x.shape[0] * 1.0)
