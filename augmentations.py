...
def random_erase(data, opt):
    rec_er = K.RandomErasing(p=0.5, scale=(.02, .4), ratio=(.3, 1/.3))
...
def shot_aug(data_support, labels_support, n_support, method, opt, instance_aug_module=None, random_crop=None, random_erase=None):
    size = data_support.shape
    if method == "instance":
        n_support = opt.s_du * n_support
        e, b = data_support.shape[0], data_support.shape[1]
        data_shot = data_support.view((-1, data_support.shape[2], data_support.shape[3], data_support.shape[4]))
        data_shot, logprobs, entropy_every, _ = instance_aug_module(data_shot)
        data_shot = data_shot.view((e, b, data_shot.shape[1], data_shot.shape[2], data_shot.shape[3]))
        data_support = torch.cat((data_support, data_shot), dim = 1)
        labels_support = torch.cat((labels_support, labels_support), dim = 1)
        return data_support, labels_support, n_support, logprobs, entropy_every
    elif method == "random_crop":
        n_support = opt.s_du * n_support
        e, b = data_support.shape[0], data_support.shape[1]
        data_shot = data_support.view((-1, data_support.shape[2], data_support.shape[3], data_support.shape[4]))
        data_shot = random_crop(data_shot)
        data_shot = data_shot.view((e, b, data_shot.shape[1], data_shot.shape[2], data_shot.shape[3]))
        data_support = torch.cat((data_support, data_shot), dim = 1)
        labels_support = torch.cat((labels_support, labels_support), dim = 1)
        ...
        e, b = data_support.shape[0], data_support.shape[1]
        data_shot = data_support.view((-1, data_support.shape[2], data_support.shape[3], data_support.shape[4]))
        data_shot = random_erase(data_shot)
        data_shot = data_shot.view((e, b, data_shot.shape[1], data_shot.shape[2], data_shot.shape[3]))
        data_support = torch.cat((data_support, data_shot), dim = 1)
        labels_support = torch.cat((labels_support, labels_support), dim = 1)
        return data_support, labels_support, n_support
...
def data_aug_mix(data, labels, r, method, opt):
    ...
    if method == "mixup" and p < r:
        ...
        for ii in range(opt.episodes_per_batch):
            ...
            rand_index = torch.randperm(data[ii].size()[0]).to(DEVICE)
