...
from augmentations import DEVICE 
...
def R2D2_Woodbury(query, support, support_labels, n_way, n_shot, l2_regularizer_lambda=50.0):
    ...
    id_matrix = torch.eye(n_support).expand(tasks_per_batch, n_support, n_support).to(DEVICE)
...
def MetaOptNetHead_SVM_He(query, support, support_labels, n_way, n_shot, C_reg=0.01, double_precision=False):
    ...
    V = (support_labels * n_way - torch.ones(tasks_per_batch, n_support, n_way).to(DEVICE)) / (n_way - 1)
    ...
    if double_precision:
        ...
        G, e, C, h = [x.to(DEVICE) for x in [G, e, C, h]]
...
def MetaOptNetHead_SVM_WW(query, support, support_labels, n_way, n_shot, C_reg=0.00001, double_precision=False):
    ...
    kernel_matrix = computeGramMatrix(support, support) + torch.ones(tasks_per_batch, n_support, n_support).to(DEVICE)
    ...
    kernel_matrix_mask_second_term = kernel_matrix_mask_second_term == torch.arange(n_way).long().repeat(n_support).reshape(n_support, n_way).transpose(1, 0).reshape(1, -1).repeat(n_support, 1).to(DEVICE)
    ...
    C_mat = C_reg * torch.ones(tasks_per_batch, n_way * n_support).to(DEVICE) - C_reg * Y_support

    C = Variable(torch.cat((id_matrix, -id_matrix), 1))
    #C = Variable(torch.cat((id_matrix_masked, -id_matrix_masked), 1))
    zer = torch.zeros(tasks_per_batch, n_way * n_support).to(DEVICE)
    ...
    # Compute the classification score.
    compatibility = computeGramMatrix(support, query) + torch.ones(tasks_per_batch, n_support, n_query).to(DEVICE)
    compatibility = compatibility.float()
...
def R2D2_Woodbury_Mixup(query, support, support_labels_a, support_labels_b, lam, n_way, n_shot, l2_regularizer_lambda=50.0):
    ...
    lam = torch.tensor(lam).float().to(DEVICE)
    support_labels_one_hot = lam.view(-1, 1, 1) * support_labels_a_one_hot + (1 - lam).view(-1, 1, 1) * support_labels_b_one_hot
