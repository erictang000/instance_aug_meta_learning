...
def mixup_criterion(opt, pred, y_a, y_b, lam):
    ...
    loss = loss_a.view(b,m,-1) * lam.view(b,m,1).to(DEVICE) + loss_b.view(b,m,-1) * (1 - lam).view(b,m,1).to(DEVICE)
...
def data_aug_(data_support, labels_support, data_query, labels_query, r, method, m, b, opt):
    ...
    if method == "qcm":
        for ii in range(b):
            ...
            rand_index = torch.randperm(data_query[ii].size()[0]).to(DEVICE)
        ...
        for ii in range(b):
            ...
            lll = np.random.beta(2., 2.)
            rand_index = torch.randperm(new_data_q[ii].size()[0]).to(DEVICE)
            label_a[ii] = labels_query[ii]
            label_b[ii] = labels_query[ii][rand_index]
...
if __name__ == '__main__':
    ...
    for epoch in range(1, opt.num_epoch + 1):
        ...
        for i, batch in enumerate(dloader_train(epoch), 1):
            ...
            new_data_s, new_data_q, label_a, label_b, ls, labels_support = new_data_s.to(DEVICE), new_data_q.to(DEVICE), label_a.to(DEVICE), label_b.to(DEVICE), ls.to(DEVICE), labels_support.to(DEVICE)
