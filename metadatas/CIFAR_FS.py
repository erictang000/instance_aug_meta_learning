...
class CIFAR_FS(ProtoData):
    def __init__(self, phase='train', augment='null', rot90_p=0., batch_size_down=8e4, random_cropping=True, color_jitter=True):
        ...
        if self.augment == 'null':
            ...
            if random_cropping:
                ...
                transform_list.append(transforms.RandomResizedCrop(32, scale=[0.1, 1]))
            if color_jitter:
                print("using random color jitter")
                transform_list.append(transforms.ColorJitter(brightness=0.4, contrast=0.4, saturation=0.4))
            transform_list.extend([transforms.RandomHorizontalFlip(), transforms.ToTensor(), normalize])
